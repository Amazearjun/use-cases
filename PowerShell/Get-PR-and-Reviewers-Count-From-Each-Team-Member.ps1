...
# Define your team members (Dragon team's 20 members)
$teamMembers = @(   
    "nani@email.com"
...
foreach ($member in $teamMembers) {
    ...
    $prs = az repos pr list --creator $member --status completed --organization $organization --project $project --output json | ConvertFrom-Json | Where-Object {$_.creationDate -ge $startDate -and $_.creationDate -le $endDate}
    ...
    if ($prs -and $prs.Count -gt 0) 
    {
        foreach ($pr in $prs) 
        {
            ...
            if ($specificRepositories -contains $repoName) 
            {
                ...
                $threadsResult = az devops invoke --area git --resource pullRequestThreads --org "$organization" --route-parameters project="$project" repositoryId="$repoId" pullRequestId="$($pr.pullRequestId)" --api-version 7.0 --output json | ConvertFrom-Json
                                
                # Count comments from reviewers (excluding the PR creator)
                $reviewerComments = 0
                $commentors = @{}

                if ($threadsResult -and $threadsResult.value) 
                {
